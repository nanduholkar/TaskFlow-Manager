...
import {SIDE_MENU_ADMIN, SIDE_MENU_USER} from "../../utils/data"
...
const SideMenu = ({activeMenu} ) => {
    ...
    useEffect(() => {
        if(user){
            setSideMenuData(user?.role === "admin" ? SIDE_MENU_ADMIN : SIDE_MENU_USER)
        }
        return () => {}
    }, [user])
    ...
    return (
      <div className="w-64 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 sticky top-[61px] z-20">
        <div className="flex flex-col items-center justify-center mb-7 pt-5">
          ...
          <h5 className="text-grey-950 font-medium leading-6 mt-3">{user?.name || ""}</h5>
